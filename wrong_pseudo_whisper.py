        ...
        try:
            for i in range(self.teacher_forcing_step-3):
                ...
                if teacher_token == 50257:
                    # =====================testing point: 懲罰模型在中間斷掉
                    loss += (1/(1+10*torch.exp(-e_loss))) * e_loss * args.em_coef + c_loss * (1 - args.em_coef) 
                elif next_tokens == 50257:
                    loss += (1/(1+10*torch.exp(-e_loss))) * e_loss * args.em_coef + c_loss * (1 - args.em_coef) + args.p_ratio * p_loss
